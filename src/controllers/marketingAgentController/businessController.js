...
const viewAllSubscribedBusinesses = async (req, res) => {
  try {
    const subscribedBusinesses = await businessModel.find({ subscribed: 'Subscribed' }).populate('businessOwner');
    return res.status(200).json({ businesses: subscribedBusinesses });
  } catch (error) {
    console.log(error);
